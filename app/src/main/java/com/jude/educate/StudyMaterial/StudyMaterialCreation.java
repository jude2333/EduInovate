...
import com.jude.educate.databinding.ActivityStudyMaterialCreationBinding;

import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.Locale;

public class StudyMaterialCreation extends AppCompatActivity {

    ActivityStudyMaterialCreationBinding binding;
...
//        setContentView(R.layout.activity_assignment_creation);
        binding = ActivityStudyMaterialCreationBinding.inflate(getLayoutInflater());
...
//        int mark = Integer.parseInt(marks);
        ...
        // get a reference to Firebase Realtime Database for generating studyMaterialsId
        DatabaseReference studyMaterialsRef = FirebaseDatabase.getInstance().getReference("materials");
        String studyMaterialsRefID = studyMaterialsRef.push().getKey();

        // upld the PDF file first
        uploadPdfToFirebase(selectedPdfUri, studyMaterialsRefID, title, description, facultyUid, currentDate);
    }


    private void uploadPdfToFirebase(Uri fileUri, String materialId, String title, String description, String facultyUid, String currentDate) {
        ...
        StorageReference storageRef = FirebaseStorage.getInstance().getReference("materials/" + materialId + "/" + getFileName(fileUri));
        ...
        storageRef.putFile(fileUri)
                .addOnSuccessListener(taskSnapshot -> {
                    ...
                    storageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                        ...
                        Material material = new Material(title, description, facultyUid, currentDate, pdfLink,courseId,materialId);

                        DatabaseReference assignmentRef = FirebaseDatabase.getInstance().getReference("materials").child(materialId);
                        assignmentRef.setValue(material)
                                .addOnSuccessListener(aVoid -> {
                                    Toast.makeText(StudyMaterialCreation.this, "Assignment uploaded successfully!", Toast.LENGTH_SHORT).show();
                                    progressDialog.dismiss();
                                    finish();
                                })
                                .addOnFailureListener(e -> {
                                    Log.e("AssignmentCreation", "Failed to save assignment to the database", e);
                                    Toast.makeText(StudyMaterialCreation.this, "Failed to save assignment to database", Toast.LENGTH_SHORT).show();
                                    progressDialog.dismiss();
                                });

                    }).addOnFailureListener(e -> {
                        Log.e("AssignmentCreation", "Failed to retrieve download URL", e);
                        Toast.makeText(StudyMaterialCreation.this, "Failed to retrieve download URL", Toast.LENGTH_SHORT).show();
                        progressDialog.dismiss();
                    });
                })
                .addOnFailureListener(e -> {
                    Log.e("AssignmentCreation", "Failed to upload PDF", e);
                    Toast.makeText(StudyMaterialCreation.this, "Failed to upload PDF", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                })
